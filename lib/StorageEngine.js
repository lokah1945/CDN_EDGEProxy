...
/**
 * CDN EdgeProxy v6.2.0 — StorageEngine
 *
 * Bug fixes from v6.1.0 carried forward:
 ...
 *
 * New enterprise features (v6.1.0):
 ...
 *
 * v6.2.0 upgrades (Section A):
 *  A1: GDSF Eviction — Greedy-Dual-Size-Frequency scoring replaces LRU
 *  A2: Blob Compression — gzip cold storage for blobs > 10KB
 *  A3: Cache Partitioning — per-origin size budgets with targeted eviction
 *  A4: Index Compaction — remove low-value entries (hitCount < 2 AND age > 7d)
 *  A5: accessCount tracking — recordHit() increments meta.hitCount via cacheKey
 *  A6: Cache-Control: immutable — returns 1-year TTL
 */
...
/**
 * A6: Cache-Control: immutable support.
 * If cache-control includes "immutable", return 365 days in ms.
 */
function computeFreshness(responseHeaders, defaultMaxAge) {
  const cc = parseCacheControl(responseHeaders["cache-control"]);
  // A6: immutable → 1-year TTL
  if (cc["immutable"]) return 365 * 24 * 60 * 60 * 1000;
...
class StorageEngine {
  /**
   * @param {object} cacheConfig       { dir, maxSize, maxAge, maxEntrySize, originBudgets }
  ...
  constructor(cacheConfig, memoryConfig = {}, concurrencyConfig = {}) {
    ...
    // A3: Origin budgets — configurable via cacheConfig.originBudgets
    // Default: ad=40%, thirdparty=50%, document=10% of maxSize
    this._originBudgets = cacheConfig.originBudgets || { ad: 0.4, thirdparty: 0.5, document: 0.1 };
  ...
  /**
   * A2: _preloadHotBlobs must handle decompression.
   * Blobs on disk may be compressed; hot cache always stores uncompressed.
   */
  async _preloadHotBlobs() {
    ...
    for (const [, meta] of this.index) {
      ...
      if (!this.hotBlobs.has(meta.blobHash)) {
        try {
          let buf = await fsp.readFile(this._blobPath(meta.blobHash));
          // A2: decompress if stored compressed
          if (meta.compressed) {
  ...
  /**
   * A4: Index compaction on stale cleanup.
   * Removes entries that exceed staleTTL OR (hitCount < 2 AND age > 7 days).
   */
  async _cleanStaleEntries() {
    ...
    for (const [key, meta] of this.index) {
      ...
      // Original stale condition
      if (age > this.staleTTL) {
        staleKeys.push([key, meta]);
        continue;
      }
      // A4: also remove low-value entries: hitCount < 2 AND age > 7 days
      const hitCount = meta.hitCount || 0;
      if (hitCount < 2 && age > sevenDays) {
        staleKeys.push([key, meta]);
      }
    }
  ...
  /**
   * A2: _blobHash always hashes the ORIGINAL (uncompressed) body.
   * The blobPath filename is based on the hash of original body.
   */
  _blobHash(buffer) {
  ...
  /**
   * A1: Increment hitCount on every HIT path access.
   */
  peekMetaAllowStale(cacheKey) {
    ...
    if (age < this.staleTTL) {
      // A1: increment hitCount and update lastAccessedAt on hit
  ...
  /**
   * A1: Increment hitCount on alias hit path.
   */
  peekAlias(aliasKey) {
   ...
   * A2: Decompress on read if meta.compressed is set.
   * Hot blobs are always uncompressed (stored uncompressed in LRU).
   */
  async getBlob(blobHash, meta) {
    // 1. Try hot in-memory cache first (always uncompressed)
    ...
    try {
      ...
      // A2: decompress if stored compressed; hot cache receives uncompressed
   ...
   * A2: Handles decompression for compressed blobs.
   * @deprecated Use getBlob() (async) instead.
   */
  getBlobSync(blobHash, meta) {
    const hot = this.hotBlobs.get(blobHash);
    if (hot) return hot;
  ...
  async put(cacheKey, url, body, headers, resourceType, origin, aliasKey, requestHeaders) {
    ...
    // A2: Compress if body > 10KB
    let bodyToWrite = body;
    let compressed  = false;
    if (body.length > 10240) {
      bodyToWrite = zlib.gzipSync(body);
      compressed  = true;
    }
    ...
    if (isNewBlob) {
      ...
      // A2: write compressed data to disk if applicable
      await fsp.writeFile(tmpPath, bodyToWrite);
      await fsp.rename(tmpPath, blobPath);
    }

    // A2: hot cache always stores UNCOMPRESSED body (for speed)
    ...
    // A1: initialise hitCount and lastAccessedAt for GDSF scoring
  ...
  async putDocument(cacheKey, url, body, headers) {
    ...
    // A2: hot cache always stores UNCOMPRESSED body
    this.hotBlobs.set(hash, body);
  ...
  /**
   * A1: GDSF scoring function.
   * score = (meta.hitCount * 1.0) / Math.max(meta.size, 1) + (meta.storedAt / 1e13)
   * Lower score = evict first.
   * The (storedAt / 1e13) aging factor prevents brand-new entries from being
   * immediately evicted while still weighting frequently-hit entries higher.
   */
  _gdsf(meta) {
    const hitCount = meta.hitCount || 0;
    const size     = Math.max(meta.size || 1, 1);
    return (hitCount * 1.0) / size + (meta.storedAt / 1e13);
  }

  // ── Eviction (A1 + A3 + BUG 4) ────────────────────────────────────────────

  /**
   * A1: Sort by GDSF score ascending (lowest = evict first) instead of storedAt.
   * A3: If an origin exceeds its budget, only evict from that origin first.
   *     Fallback: if no budgets configured, use global eviction.
   * BUG 4: O(1) refcount-based blob deletion.
   */
  _evictIfNeeded() {
    let totalSize = 0;
    ...
    // A3: Check per-origin budgets first
    const originBudgets = this._originBudgets;
    let handledByOrigin = false;

    if (originBudgets && Object.keys(originBudgets).length > 0) {
      for (const [origin, fraction] of Object.entries(originBudgets)) {
        const budget    = this.maxSize * fraction;
        const originSize = this._getOriginSize(origin);
        if (originSize > budget) {
          log.info("Storage", `Origin "${origin}" exceeds budget (${(originSize / 1024 / 1024).toFixed(1)}MB > ${(budget / 1024 / 1024).toFixed(1)}MB), evicting from origin`);
          // Evict only from this origin, sorted by GDSF ascending
          const originEntries = [...this.index.entries()]
            .filter(([, m]) => m.origin === origin)
            .sort((a, b) => this._gdsf(a[1]) - this._gdsf(b[1]));

          let evicted = 0;
          let currentOriginSize = originSize;
          while (currentOriginSize > budget * 0.9 && originEntries.length > 0) {
            const [key, meta] = originEntries.shift();
            currentOriginSize -= (meta.size || 0);
          ...
          if (evicted > 0) {
            handledByOrigin = true;
            log.info("Storage", `Evicted ${evicted} entries from origin "${origin}". ${this.index.size} remaining.`);
          }
        }
      }
    }

    // Recalculate totalSize after origin-specific eviction
    if (handledByOrigin) {
      totalSize = 0;
      for (const [, meta] of this.index) totalSize += (meta.size || 0);
    ...
    // A1: Global eviction — sort by GDSF score ascending (lowest = evict first)
    const entries = [...this.index.entries()].sort((a, b) => this._gdsf(a[1]) - this._gdsf(b[1]));
  ...
  /**
   * A3: Helper that sums sizes for entries of a given origin.
   */
  _getOriginSize(origin) {
    let total = 0;
    for (const [, meta] of this.index) {
      if (meta.origin === origin) total += (meta.size || 0);
    }
    return total;
  }
  ...
  /**
   * A5: recordHit() now accepts an optional cacheKey parameter.
   * If cacheKey is provided, increments meta.hitCount for the cached entry.
   * Backward compat: if cacheKey is undefined, skip hitCount increment.
   */
  recordHit(url, resourceType, origin, bytes, wireBytes, cacheKey) {
    this.stats.hits++;
    ...
    // A5: increment hitCount in meta if cacheKey is provided
  ...
  getReport() {
    ...
    let r = "";
    r += `${"═".repeat(62)}\n`;
    r += `  CDN EdgeProxy CACHE REPORT v6.2\n`;
    r += `${"═".repeat(62)}\n`;
